...
def plus_cart(request):
    if request.method == 'GET':
        ...
        data = {
            ...
            'amount': amount,
            'totalamount': amount + shipping_amount,
            'cart_count': cart_products.count()  # Corrected cart count
        }
        return JsonResponse(data)
...
def minus_cart(request):
    if request.method == 'GET':
        ...
        data = {
            'quantity': c.quantity if c.quantity > 1 else 0,
            'amount': amount,
            'totalamount': amount + shipping_amount,
            'cart_count': cart_products.count()  # Corrected cart count
        }
 ...
 #   user = request.user
  #  custid = request.GET.get('custid')
   # paymentid = request.GET.get('paymentid')
    ...
    #for c in cart:
    #   OrderPlaced(user=user, customer=customer, product=c.product, quantity=c.quantity, payment_id=paymentid).save()
    #  c.delete()
...
def search(request):
 ...
 allPosts = Product.objects.filter(title__icontains=query)
